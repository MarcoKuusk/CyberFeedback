...
def test_migrate_moves_legacy_into_campaign(store):
    ...
    ids = campaign_store.list_submissions(campaign["campaign_id"], "employee")
    assert len(ids) == 1
...
def test_migrate_both_tracks(store):
    ...
    assert len(campaign_store.list_submissions(campaign["campaign_id"], "employee")) == 1
    assert len(campaign_store.list_submissions(campaign["campaign_id"], "organization")) == 1
