...
Properties:
  ...
  - Consent-honest: migrated records carry consent.agreed = False, because these
    respondents were never asked. They are excluded from the research export.
...
def migrate() -> int:
    ...
    for track in legacy_tracks:
        ...
        # Legacy records predate consent collection, so they come in through the
        # explicit legacy door: consent is stored as NOT given, keeping them out
        # of the research dataset rather than fabricating agreement.
        respondent_id = campaign_store.import_legacy_submission(campaign_id, track, payload)
